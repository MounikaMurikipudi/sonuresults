...
with mysql.connector.connect(host=host,user=user,password=password,db=db,port=port) as conn:
    ...
    cursor.execute("create table if not exists internalresults(studentid varchar(10) primary key,courseid varchar(20) primary key,Internal1 enum('Internal1'),Internal2 enum('Internal2'),internalmarks2 smallint,internalmarks1 smallint,section enum('BCA','B.Sc M.S.Ds','BBA','BBA BA','BA','BSC MPCs','B.Sc C.A.M.E','B.Sc M.S.Cs'),foreign key(studentid) references addstu(studentid),foreign key(courseid) references addsub(courseid))")
